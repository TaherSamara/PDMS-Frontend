...
export class AddComponent {
  ...
  constructor(public publicService: PublicService, public httpService: HttpService, private api: ApiService,
    private changeDetectorRef: ChangeDetectorRef, private modalService: NgbModal, private router: Router) { }
  ...
  getLookups() {
    this.httpService.list(this.api.common.lookups, {}).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  save() {
    ...
    this.httpService.action(this.api.users.add, formData, 'addUserLoader').subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  initForm() {
    this.form = new FormGroup({
      ...
      ReceivesEmails: new FormControl('1', Validators.required),
      EmailFrequencyInHours: new FormControl('4', Validators.required)
    });
  }
}
